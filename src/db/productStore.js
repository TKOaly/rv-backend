...
const rowToProduct = (row) => {
	if (row !== undefined) {
		return {
			barcode: row.barcode,
			name: row.descr,
			category: { categoryId: row.pgrpid, description: row.pgrpdescr },
...
module.exports.searchProducts = async (query) => {
	const data = await knex('PRICE').rightJoin(
		'RVITEM',
		'PRICE.itemid',
		'RVITEM.itemid',
		...
		'PRICE.sellprice',
		'PRICE.count',
	).whereILike('RVITEM.descr', `%${query}%`).orWhereILike(
		'PRICE.barcode',
		`%${query}%`,
	);
...
module.exports.findByBarcode = async (barcode) => {
	const row = await knex('PRICE').rightJoin(
		'RVITEM',
		'PRICE.itemid',
		'RVITEM.itemid',
	).leftJoin('PRODGROUP', 'RVITEM.pgrpid', 'PRODGROUP.pgrpid').select(
		...
		'PRICE.sellprice',
		'PRICE.count',
	).where('PRICE.barcode', barcode).andWhere('PRICE.endtime', null).first();
...
module.exports.insertProduct = async (productData, userId) => {
	return await knex.transaction(async (trx) => {
		const insertedRows = await knex('RVITEM').transacting(trx).insert({
		...
		const categoryRow = await knex('PRODGROUP').transacting(trx).select(
			'descr',
		).where('pgrpid', productData.categoryId).first();
...
module.exports.updateProduct = async (barcode, productData, userId) => {
	...
	return await knex.transaction(async (trx) => {
		...
		if (Object.keys(rvitemFields).length > 0) {
			const priceRow = await knex('PRICE').transacting(trx).select(
				'itemid',
			).where({ barcode: barcode, endtime: null }).first();

			await knex('RVITEM').transacting(trx).update(rvitemFields).where({
		...
		if (Object.keys(priceFields).length > 0) {
			if (priceFields.sellPrice === undefined) {
				await knex('PRICE').transacting(trx).update(priceFields).where({
					barcode: barcode,
					endtime: null,
				});
			} else {
				...
				const currentPriceRows = await knex('PRICE').transacting(trx)
					.update({ endtime: now }).where({
						barcode: barcode,
						endtime: null,
					}).returning([
						'barcode',
		...
		const productRow = await knex('PRICE').transacting(trx).leftJoin(
			...
			'PRICE.sellprice',
			'PRICE.count',
		).where('PRICE.barcode', barcode).andWhere('PRICE.endtime', null)
			.first();
...
module.exports.recordPurchase = async (barcode, userId, count) => {
	return await knex.transaction(async (trx) => {
		...
		const updatedPriceRows = await knex('PRICE').transacting(trx).innerJoin(
			'RVITEM',
			'PRICE.itemid',
			'RVITEM.itemid',
		).andWhere('barcode', barcode).andWhere('endtime', null).decrement({
			count: count,
		...
		const updatedPersonRows = await knex('RVPERSON').transacting(trx).where(
			{ userid: userId },
		).decrement({ saldo: count * price }).returning(['saldo']);
		...
		for (let i = 0; i < count; i++) {
			...
			const insertedSaldhistRows = await knex('SALDOHISTORY').transacting(
				trx,
			).insert({
				userid: userId,
				time: now,
			...
			const insertedItemhistRows = await knex('ITEMHISTORY').transacting(
				trx,
			).insert({
				time: now,
				count: stock,
...
module.exports.deleteProduct = async (barcode) => {
	return await knex.transaction(async (trx) => {
		const row = await knex('PRICE').transacting(trx).leftJoin(
			'RVITEM',
			'PRICE.itemid',
			'RVITEM.itemid',
		).leftJoin('PRODGROUP', 'RVITEM.pgrpid', 'PRODGROUP.pgrpid').select(
		...
		await knex('RVITEM').transacting(trx).where({ itemid: row.itemid })
			.update({ deleted: true });
...
module.exports.buyIn = async (barcode, count) => {
	const row = await knex('PRICE').where({ barcode }).increment({ count })
		.returning(['count']);
