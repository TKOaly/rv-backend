...
const createPurchaseHistoryQuery = () =>
	knex('ITEMHISTORY').leftJoin(
		'RVITEM',
		'ITEMHISTORY.itemid',
		'RVITEM.itemid',
	).leftJoin('PRODGROUP', 'RVITEM.pgrpid', 'PRODGROUP.pgrpid').leftJoin(
		'PRICE',
		'ITEMHISTORY.priceid1',
		'PRICE.priceid',
	).leftJoin('RVPERSON', 'ITEMHISTORY.userid', 'RVPERSON.userid').leftJoin(
		'ROLE',
		'RVPERSON.roleid',
		'ROLE.roleid',
	).leftJoin(
		...
		'ROLE.role',
		'SALDOHISTORY.saldo',
	).where('ITEMHISTORY.actionid', 5) /* actionid 5 = buy action */.orderBy([{
		column: 'ITEMHISTORY.time',
		order: 'desc',
	}, { column: 'ITEMHISTORY.itemhistid', order: 'desc' }]);

const createDepositHistoryQuery = () =>
	knex('PERSONHIST').leftJoin(
		'SALDOHISTORY',
		'PERSONHIST.saldhistid',
		'SALDOHISTORY.saldhistid',
	).leftJoin('RVPERSON', 'PERSONHIST.userid1', 'RVPERSON.userid').leftJoin(
		'ROLE',
		'RVPERSON.roleid',
		'ROLE.roleid',
	).select(
		...
		.orderBy([{ column: 'PERSONHIST.time', order: 'desc' }, {
			column: 'PERSONHIST.pershistid',
			order: 'desc',
		}]);
...
module.exports.getUserPurchaseHistory = async (userId) => {
	...
	return data.map((row) => {
		return { ...rowToPurchase(row), product: rowToProduct(row) };
	});
};
...
module.exports.getProductPurchaseHistory = async (barcode) => {
	...
	return data.map((row) => {
		return { ...rowToPurchase(row), user: rowToUser(row) };
	});
};

module.exports.findPurchaseById = async (purchaseId) => {
	const row = await createPurchaseHistoryQuery().andWhere(
		'ITEMHISTORY.itemhistid',
		purchaseId,
	).first();
...
module.exports.getDepositHistory = async () => {
	...
	return data.map((row) => {
		return { ...rowToDeposit(row), user: rowToUser(row) };
	});
};
...
module.exports.findDepositById = async (depositId) => {
	const row = await createDepositHistoryQuery().andWhere(
		'PERSONHIST.pershistid',
		depositId,
	).first();
