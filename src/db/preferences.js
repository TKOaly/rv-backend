...
module.exports.getPreference = async (preference) => {
	const data = await knex('PREFERENCES').where({ key: preference.key }).first(
		'value',
	);
...
module.exports.setPreference = async (preference, value) => {
	...
	if (preference.validate) {
		...
		if (errors.length > 0) {
			return { previousValue: undefined, value, errors };
		}
	}
	...
	if (data === undefined) {
		...
		return { errors: [], value, previousValue: undefined };
	} else {
		await knex('PREFERENCES').update({ value: serialized }).where({
			key: preference.key,
		});

		return { errors: [], value, previousValue: data.value };
	}
};
...
module.exports.getPreferenceByKey = (key) => {
	return Object.values(module.exports.preferences).find((pref) =>
		pref.key === key
	);
};
