...
module.exports.getUsers = async () => {
	const data = await knex('RVPERSON').leftJoin(
		'ROLE',
		'RVPERSON.roleid',
		'ROLE.roleid',
	).select(
...
module.exports.findById = async (userId) => {
	const row = await knex('RVPERSON').leftJoin(
		'ROLE',
		'RVPERSON.roleid',
		'ROLE.roleid',
	).select(
		...
		'RVPERSON.pass',
		'RVPERSON.rfid',
	).where('RVPERSON.userid', userId).first();
...
module.exports.findByRfid = async (rfid) => {
	...
	const row = await knex('RVPERSON').leftJoin(
		...
		'RVPERSON.pass',
		'RVPERSON.rfid',
	).where('RVPERSON.rfid', rfid_hash).first();
	return rowToUser(row);
};
module.exports.findByUsername = async (username) => {
	const row = await knex('RVPERSON').leftJoin(
		...
		'RVPERSON.pass',
		'RVPERSON.rfid',
	).where('RVPERSON.name', username).first();
	return rowToUser(row);
};
...
module.exports.findByEmail = async (email) => {
	const row = await knex('RVPERSON').leftJoin(
		...
		'RVPERSON.pass',
		'RVPERSON.rfid',
	).where('RVPERSON.univident', email).first();
	return rowToUser(row);
};
...
module.exports.insertUser = async (userData) => {
	...
	const insertedPersonRows = await knex('RVPERSON').insert({
		createdate: new Date(),
		// roleid 2 = USER1
		roleid: 2,
...
module.exports.updateUser = async (userId, userData) => {
	return await knex.transaction(async (trx) => {
		...
		if (userData.role !== undefined) {
			const roleRow = await knex('ROLE').transacting(trx).select('roleid')
				.where({ role: userData.role }).first();
			rvpersonFields.roleid = roleRow.roleid;
		}
		await knex('RVPERSON').transacting(trx).update(rvpersonFields).where({
			userid: userId,
		});

		const userRow = await knex('RVPERSON').transacting(trx).leftJoin(
			'ROLE',
			'RVPERSON.roleid',
			'ROLE.roleid',
		).select(
...
module.exports.recordDeposit = async (userId, amount) => {
	return await knex.transaction(async (trx) => {
		...
		const updatedPersonRows = await knex('RVPERSON').transacting(trx).where(
			{ userid: userId },
		).increment({ saldo: amount }).returning(['saldo']);

		const insertedSaldhistRows = await knex('SALDOHISTORY').transacting(trx)
			.insert({
				userid: userId,
				time: now,
		...
		const insertedPershistRows = await knex('PERSONHIST').transacting(trx)
			.insert({
				time: now,
				actionid: 17,
				userid1: userId,
