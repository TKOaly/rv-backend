exports.up = async (knex) => {
	const itemhistory = await knex('ITEMHISTORY').select('saldhistid').where(
		'actionid',
		5,
	);
	const saldohistory = await knex('SALDOHISTORY').select(
		'saldhistid',
		'time',
		'userid',
	).orderBy(['time', 'userid']);
	const personhist = await knex('PERSONHIST').select(
		'pershistid',
		'time',
		'userid1',
	).where('actionid', 17).orderBy(['time', 'userid1']);
	...
	const unconnectedSaldoEvents = saldohistory.filter((saldoEvent) =>
		!connectedSaldoIds.has(saldoEvent.saldhistid)
	);
	...
	await knex.schema.table('PERSONHIST', (table) => {
		table.integer('saldhistid').references('saldhistid').inTable(
			'SALDOHISTORY',
		).defaultTo(null);
	});
	...
	for (const depositEvent of personhist) {
		await knex('PERSONHIST').update('saldhistid', depositEvent.saldhistid)
