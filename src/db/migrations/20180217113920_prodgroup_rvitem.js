exports.up = async (knex) => {
	if (!(await knex.schema.hasTable('PRODGROUP'))) {
		await knex.schema.createTable('PRODGROUP', (table) => {
			...
			table.string('descr', 64).notNullable().comment(
	...
	if (!(await knex.schema.hasTable('RVITEM'))) {
		await knex.schema.createTable('RVITEM', (table) => {
			...
			table.integer('pgrpid').unsigned().notNullable().comment(
				'Reference to product group of this item',
			).references('pgrpid').inTable('PRODGROUP');
			table.string('descr', 64).notNullable().comment(
				'Textual product description (name)',
			);
		});
	}
};
...
exports.down = async (knex) => {
	if (process.env.NODE_ENV !== 'production') {
		await knex.schema.dropTableIfExists('RVITEM').dropTableIfExists(
			'PRODGROUP',
		);
	} else {
