...
router.get('/', async (req, res) => {
	...
	try {
		...
		res.status(200).json({ boxes: mappedBoxes });
	} catch (error) {
...
router.post('/', async (req, res) => {
	...
	res.status(201).json({ box: mapDatabaseBoxToApiBox(newBox) });
});
...
router.get('/:boxBarcode(\\d{1,14})', async (req, res) => {
	...
	res.status(200).json({ box: mapDatabaseBoxToApiBox(box) });
});
...
router.patch('/:boxBarcode(\\d{1,14})', async (req, res) => {
	...
	if (productBarcode !== undefined) {
		const existingProduct = await productStore.findByBarcode(
			productBarcode,
		);
		if (!existingProduct) {
			logger.error(
	...
	const updatedBox = await boxStore.updateBox(
		boxBarcode,
		deleteUndefinedFields({ itemsPerBox, productBarcode }),
	);
	...
	res.status(200).json({ box: mapDatabaseBoxToApiBox(updatedBox) });
});
...
router.post('/:boxBarcode(\\d{1,14})/buyIn', async (req, res) => {
	...
	const update = {
		sellPrice: oldsellprice !== productSellPrice
			? productSellPrice
			: undefined,
