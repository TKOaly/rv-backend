...
router.get(
	...
	async (req, res) => {
		...
		try {
			...
			logger.info(
				'User %s fetched products as admin',
				user.username,
			);
			res.status(200).json({
...
router.get(
	...
	async (req, res) => {
		...
		logger.info(
			'User %s fetched product %s as admin',
			user.username,
			barcode,
		);
...
router.post(
	'/:barcode(\\d{1,14})/buyIn',
	async (req, res) => {
		const barcode = req.params.barcode;
		const { count, buyPrice, sellPrice } = req.body;

		const stock = await productStore.buyIn(
			barcode,
			count,
		);
...
router.get(
	'/:barcode(\\d{1,14})/purchaseHistory',
	async (req, res) => {
		const barcode = req.params.barcode;
