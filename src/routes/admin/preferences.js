...
const { preferences, getPreference, setPreference, getPreferenceByKey } =
...
router.get('/', async (req, res) => {
	...
	res.status(200).json({ preferences: values });
});
...
router.get('/:preferenceKey', async (req, res) => {
	...
	if (preference === undefined) {
		res.status(404).json({
			...
			message:
				`No preference with key '${req.params.preferenceKey}' exists`,
		});
	...
	res.status(200).json({
		preference: { key: req.params.preferenceKey, value },
	});
});
...
router.patch('/:preferenceKey', async (req, res) => {
	...
	res.status(200).json({
		preference: { key: req.params.preferenceKey, value: result.value },
	});
});
