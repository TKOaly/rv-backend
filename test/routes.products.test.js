...
describe(
	...
	() => {
		...
		describe(
			...
			() => {
				it(
					'should return matching product if found',
					async () => {
				...
				it(
					'should return no matching product if not found',
					async () => {
						const res = await chai
							.request(server)
							.post('/api/v1/products/search')
		...
		describe(
			...
			() => {
				...
				it(
					'should return 404 on nonexistent product',
					async () => {
						const res = await chai
							.request(server)
		...
		describe(
			...
			() => {
				it(
					'should deduct account balance and product stock',
					async () => {
				...
				it(
					'should create an event into purchase history',
					async () => {
				...
				it(
					'should create multiple history events on multibuy',
					async () => {
						const user = await userStore.findByUsername('normal_user');
						const oldPurchaseHistory =
                await historyStore.getUserPurchaseHistory(user.userId);
				...
				it(
					'should error on insufficient funds',
					async () => {
						const user = await userStore.findByUsername('normal_user');
