...
describe('routes: products', () => {
	...
	describe('Fetching all products', () => {
		it('should return all products', async () => {
			const res = await chai.request(server).get('/api/v1/products').set(
				'Authorization',
				'Bearer ' + token,
			);
	...
	describe('Purchasing product', () => {
		it('should deduct account balance and product stock', async () => {
			...
			const oldProduct = await productStore.findByBarcode(
				'8855702006834',
			);

			const res = await chai.request(server).post(
				'/api/v1/products/8855702006834/purchase',
			).set('Authorization', 'Bearer ' + token).send({ count: 1 });

			expect(res.status).to.equal(200);

			const newUser = await userStore.findByUsername('normal_user');
			const newProduct = await productStore.findByBarcode(
				'8855702006834',
			);
		...
		it('should create an event into purchase history', async () => {
			...
			const oldPurchaseHistory = await historyStore
				.getUserPurchaseHistory(user.userId);

			const res = await chai.request(server).post(
				'/api/v1/products/6417901011105/purchase',
			).set('Authorization', 'Bearer ' + token).send({ count: 1 });

			expect(res.status).to.equal(200);

			const newPurchaseHistory = await historyStore
				.getUserPurchaseHistory(user.userId);
		...
		it('should create multiple history events on multibuy', async () => {
			...
			const res = await chai.request(server).post(
				'/api/v1/products/6417901011105/purchase',
			).set('Authorization', 'Bearer ' + token).send({ count: 3 });
		...
		it('should return 404 on nonexistent product', async () => {
			const res = await chai.request(server).post(
				'/api/v1/products/1234567890123/purchase',
			).set('Authorization', 'Bearer ' + token).send({ count: 1 });
