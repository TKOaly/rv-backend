...
describe('routes: user', () => {
	...
	describe('Fetching user info', () => {
		it('should return user info', async () => {
			const res = await chai.request(server).get('/api/v1/user').set(
				'Authorization',
				'Bearer ' + token,
			);
	...
	describe('Modifying user info', () => {
		it('should modify user', async () => {
			const res = await chai.request(server).patch('/api/v1/user').set(
				'Authorization',
				'Bearer ' + token,
			).send({
				username: 'abcd',
	...
	describe('Depositing money', () => {
		it('should increase account balance', async () => {
			const res = await chai.request(server).post('/api/v1/user/deposit')
		...
		it('should create an event into deposit history', async () => {
			...
			const res = await chai.request(server).post('/api/v1/user/deposit')
				.set('Authorization', 'Bearer ' + token).send({ amount: 2371 });
		...
		it('should error on depositing a negative amount', async () => {
			const res = await chai.request(server).post('/api/v1/user/deposit')
				.set('Authorization', 'Bearer ' + token).send({ amount: -200 });
