...
describe('routes: admin users', () => {
	...
	describe('Fetching all users', () => {
		it('should return all users', async () => {
			const res = await chai.request(server).get('/api/v1/admin/users')
	...
	describe('Fetching user by id', () => {
		it('should return the user', async () => {
			const res = await chai.request(server).get('/api/v1/admin/users/1')
				.set('Authorization', 'Bearer ' + token);
		...
		it('should error on nonexistent user', async () => {
			const res = await chai.request(server).get('/api/v1/admin/users/77')
				.set('Authorization', 'Bearer ' + token);
	...
	describe('Changing user role', () => {
		it('should change the role', async () => {
			const res = await chai.request(server).post(
				'/api/v1/admin/users/1/changeRole',
		...
		it('should error on nonexistent user', async () => {
			const res = await chai.request(server).post(
				'/api/v1/admin/users/99/changeRole',
			).set('Authorization', 'Bearer ' + token).send({ role: 'ADMIN' });
		...
		it('should error on invalid role', async () => {
			const res = await chai.request(server).post(
				'/api/v1/admin/users/1/changeRole',
			).set('Authorization', 'Bearer ' + token).send({ role: 'abc' });
	...
	describe('Fetching user\'s deposit history', async () => {
		it('should return list of deposits', async () => {
			const res = await chai.request(server).get(
				'/api/v1/admin/users/1/depositHistory',
			).set('Authorization', 'Bearer ' + token);
	...
	describe('Fetching user\'s purchase history', async () => {
		it('should return a list of purchases', async () => {
			const res = await chai.request(server).get(
				'/api/v1/admin/users/1/purchaseHistory',
			).set('Authorization', 'Bearer ' + token);
