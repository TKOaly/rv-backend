...
describe(
	...
	() => {
		...
		describe(
			...
			() => {
				it(
					'should return a list of preferences and values',
					async () => {
		...
		describe(
			"querying a preference by it's key",
			() => {
				it(
				...
				it(
					'should return a default value for an undefined preference',
					async () => {
						const res = await chai
							.request(server)
						...
						expect(res.body.preference).to.contain.all.keys(
							'value',
							'key',
						);
		...
		describe(
			...
			() => {
				...
				it(
					'should cause queries to resolve with the new value',
					async () => {
						const res = await chai
							.request(server)
				...
				it(
					'should fail when setting an invalid value',
					async () => {
						const res = await chai
							.request(server)
							.patch('/api/v1/admin/preferences/globalDefaultMargin')
