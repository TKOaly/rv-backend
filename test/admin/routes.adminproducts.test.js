	...
	() => {
		...
		describe(
			...
			() => {
				...
				it(
					'should error on nonexistent product',
					async () => {
						const res = await chai
							.request(server)
		...
		describe(
			...
			() => {
				...
				it(
					'should error if barcode is already taken',
					async () => {
						const res = await chai
							.request(server)
							.post('/api/v1/admin/products')
				...
				it(
					'should error on nonexistent category',
					async () => {
						const res = await chai
							.request(server)
							.post('/api/v1/admin/products')
		...
		describe(
			...
			() => {
				...
				it(
					'should allow modifying only some fields',
					async () => {
						const res = await chai
							.request(server)
							.patch('/api/v1/admin/products/5053990123506')
		...
		describe(
			...
			() => {
				it(
					'should fail on nonexisting product',
					async () => {
						const res = await chai
							.request(server)
				...
				it(
					"should cause any requests for that product's information to fail",
					async () => {
						await chai
							.request(server)
							.delete('/api/v1/admin/products/5053990123506')
				...
				it(
					'should cause the item to be removed from item listing',
					async () => {
						await chai
							.request(server)
							.delete('/api/v1/admin/products/5053990123506')
				...
				it(
					'should prevent the item from being purchased',
					async () => {
						await chai
							.request(server)
							.delete('/api/v1/admin/products/5053990123506')
		...
		describe(
			...
			() => {
				it(
					'should fail on nonexisting products',
					async () => {
						const res = await chai
							.request(server)
				...
				it(
					'should fail on invalid request',
					async () => {
						const validRequest = {
							count: 1,
							buyPrice: 1,
				...
				it(
					'should change the number of products in stock',
					async () => {
				...
				it(
					"should change the item's buyPrice and sellPrice",
					async () => {
						const pre_query = await chai
							.request(server)
							.get('/api/v1/admin/products/5053990123506')
		...
		describe(
			"Querying product's purchase history",
			() => {
				it(
