...
const normalUserToken = jwt.sign({ userId: 1 }, process.env.JWT_SECRET);
...
describe('routes: admin products', () => {
	...
	describe('Fetching all products', () => {
		it('should return all products', async () => {
			const res = await chai.request(server).get('/api/v1/admin/products')
	...
	describe('Fetching product by barcode', () => {
		it('should return the product', async () => {
			const res = await chai.request(server).get(
				'/api/v1/admin/products/5053990123506',
			).set('Authorization', 'Bearer ' + token);
	...
	describe('Modifying product data', () => {
		it('should modify the product', async () => {
			...
			const updatedProduct = await productStore.findByBarcode(
				'5053990123506',
			);
		...
		it('should allow modifying only some fields', async () => {
			const res = await chai.request(server).patch(
				'/api/v1/admin/products/5053990123506',
			).set('Authorization', 'Bearer ' + token).send({ buyPrice: 5000 });
		...
		it('should error on nonexistent product', async () => {
			const res = await chai.request(server).patch(
				'/api/v1/admin/products/88888888',
			).set('Authorization', 'Bearer ' + token).send({ buyPrice: 5000 });
		...
		it('should error on nonexistent category', async () => {
			const res = await chai.request(server).patch(
				'/api/v1/admin/products/5053990123506',
			).set('Authorization', 'Bearer ' + token).send({ categoryId: 999 });
		...
		it('should error on invalid parameters', async () => {
			const res = await chai.request(server).patch(
				'/api/v1/admin/products/5053990123506',
			).set('Authorization', 'Bearer ' + token).send({ aaa: 4 });
	...
	describe('Deleting a product', () => {
		...
		it('should cause any requests for that product\'s information to fail', async () => {
			await chai.request(server).delete(
				'/api/v1/admin/products/5053990123506',
			).set('Authorization', 'Bearer ' + token);

			const lookup = await chai.request(server).get(
				'/api/v1/admin/products/5053990123506',
			).set('Authorization', 'Bearer ' + token);
		...
		it('should cause the item to be removed from item listing', async () => {
			...
			const listing = await chai.request(server).get(
				'/api/v1/admin/products',
			).set('Authorization', 'Bearer ' + token);

			expect(
				listing.body.products.find((item) =>
					item.barcode === '5053990123506'
				),
			).to.be.an('undefined');
		});
		...
		it('should prevent the item from being purchased', async () => {
			...
			const purchase = await chai.request(server).post(
				'/api/v1/products/5053990123506/purchase',
			).set('Authorization', 'Bearer ' + normalUserToken).send({
				count: 1,
			});
	...
	describe('Buy-in of a product', () => {
		it('should fail on nonexisting products', async () => {
			const res = await chai.request(server).post(
				'/api/v1/admin/products/88888888/buyIn',
			).set('Authorization', 'Bearer ' + token).send({
				count: 1,
				buyPrice: 1,
				sellPrice: 1,
		...
		it('should fail on invalid request', async () => {
			const validRequest = { count: 1, buyPrice: 1, sellPrice: 1 };
			...
			for (const missingField in validRequest) {
				...
				const res = await chai.request(server).post(
					'/api/v1/admin/products/5053990123506/buyIn',
		...
		it('should change the number of products in stock', async () => {
			const pre_query = await chai.request(server).get(
				'/api/v1/admin/products/5053990123506',
			).set('Authorization', 'Bearer ' + token);
			...
			const post_query = await chai.request(server).get(
				'/api/v1/admin/products/5053990123506',
			).set('Authorization', 'Bearer ' + token);
	...
	describe('Querying product\'s purchase history', () => {
		it('should return a list of purchases', async () => {
			const res = await chai.request(server).get(
				'/api/v1/admin/products/5053990123506/purchaseHistory',
			).set('Authorization', 'Bearer ' + token);
